...
Bitmap.fromBuffer = function (buffer) {
  ...
  if (!hasError) {
    ...
    while (pos < header.dataOffset) {
      ...
      // numberOfBytesRead = fs.readSync(file, buffer, 0, buffer.length, null);
  ...
  if (!hasError) {
    ...
    buffer.copy(data, 0, 0, count);

    // buffer = new Buffer(numberOfBytesPerLine);
    // for (var y = header.height - 1; y >= 0; y--) {
    //   //noinspection JSDuplicatedDeclaration
    //   numberOfBytesRead = fs.readSync(file, buffer, 0, numberOfBytesPerLine,
    //     null);
    ...
    //     break;
    //   }
    //   buffer.copy(bitmapData, y * header.width, 0, header.width);
    // }
  }
  // Finish
  if (hasError) {
...
Bitmap.prototype.toBuffer = function () {
  ...
  var numberOfBytesPerLine = Math.ceil(this._width * this._bytesPerPixel / 4/*bytes*/) *
    4/*bytes*/;
  ...
  switch (this._bytesPerPixel) {
    case 1:
    {
      transferPixel = function () {
        buffer.writeUInt8(this._readFunction.call(this._data, sourceOffset),
          offset);
      };
      break;
    }
    ...
    {
      transferPixel = function () {
        buffer.writeUInt16LE(this._readFunction.call(this._data, sourceOffset),
          offset);
      };
      break;
    }
    ...
    {
      transferPixel = function () {
        buffer.writeUInt32LE(this._readFunction.call(this._data, sourceOffset),
          offset);
      };
      break;
    }
 ...
 * @param {string} filePath
 */
Bitmap.prototype.save = function (filePath) {
  // Write file

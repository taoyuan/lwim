...
Bitmap.fromFile = function (filePath) {
  ...
  if (!hasError) {
    ...
    while (filePosition < header.dataOffset) {
      ...
      var _numberOfBytesRead = fs.readSync(file, fileBuffer, 0, fileBuffer.length, null);
      if (_numberOfBytesRead != fileBuffer.length) {
        hasError = true;
  ...
  if (!hasError) {
    ...
    for (var y = header.height - 1; y >= 0; y--) {
      //noinspection JSDuplicatedDeclaration
      var _numberOfBytesRead2 = fs.readSync(file, fileBuffer, 0, numberOfBytesPerLine, null);
      if (_numberOfBytesRead2 != numberOfBytesPerLine) {
        hasError = true;
        errorMessage = "Unexpected end of file in \"" + filePath + "\"";
        break;
      }
...
Bitmap.prototype.save = function (filePath) {
  ...
  var fileBuffer = new Buffer(header.fileSize);
